...
impl Matrix32 {
	...
	pub fn with_scaling(mut self, scale: f32) -> Self {
		self.rot = Matrix22::scaling(scale);
		self
	}
	...
	pub fn with_translation(mut self, t: &Vector2) -> Self {
		self.pos = *t;
		self
	}
	...
	pub fn from_matrix33(m: &Matrix33) -> Self {
		Self {
			rot: Matrix22::new(&Vector2::new(m.x.x, m.x.y), &Vector2::new(m.y.x, m.y.y)),
...
impl From<[f32; 6]> for Matrix32 {
	fn from(m: [f32; 6]) -> Self {
		Self {
			rot: Matrix22::new(&Vector2::new(m[0], m[3]), &Vector2::new(m[1], m[4])),
...
impl std::fmt::Debug for Matrix32 {
	fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
		writeln!(
			f,
			"Matrix32:\n{} {} {}\n{} {} {}",
			self.rot.x.x, self.rot.x.y, self.pos.x, self.rot.y.x, self.rot.y.y, self.pos.y,
		)
	}
}
